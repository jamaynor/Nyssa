...
BEGIN
	...
	SELECT name INTO role_name FROM authorization.roles WHERE id = p_role_id;
	SELECT name INTO org_name FROM authorization.organizations WHERE id = p_organization_id;
...
-- ============================================================================
CREATE OR REPLACE FUNCTION authorization.get_user_roles(
	p_user_id				UUID,
...
BEGIN
	...
	IF p_include_inherited AND p_organization_id IS NOT NULL THEN
		...
		JOIN authorization.organizations o ON ur.organization_id = o.id
		JOIN authorization.organizations target_org ON target_org.id = p_organization_id
		WHERE ur.user_id = p_user_id
			AND ur.is_active = true
			AND r.is_active = true
			AND o.is_active = true
...
-- ============================================================================
CREATE OR REPLACE FUNCTION authorization.add_permission_to_role(
	p_role_id				UUID,
...
BEGIN
	...
	IF EXISTS(
		SELECT 1 FROM authorization.role_permissions
...
-- ============================================================================
CREATE OR REPLACE FUNCTION authorization.remove_permission_from_role(
	p_role_id				UUID,
	p_permission			VARCHAR,
	p_removed_by			UUID
)
...
BEGIN
	...
	FROM authorization.role_permissions
	WHERE role_id = p_role_id AND permission_id = permission_id;
...
-- ============================================================================
CREATE OR REPLACE FUNCTION authorization.expire_user_roles()
RETURNS TABLE(
...
-- Comments
COMMENT ON FUNCTION authorization.assign_user_role IS 'Assigns a role to a user with comprehensive validation and audit logging';
COMMENT ON FUNCTION authorization.revoke_user_role IS 'Revokes a role from a user and logs the revocation';
COMMENT ON FUNCTION authorization.get_user_roles IS 'Returns all roles assigned to a user, including inherited roles from parent organizations';
COMMENT ON FUNCTION authorization.add_permission_to_role IS 'Adds a permission to a role with validation';
COMMENT ON FUNCTION authorization.remove_permission_from_role IS 'Removes a permission from a role';
COMMENT ON FUNCTION authorization.expire_user_roles IS 'Automatically expires user roles that have passed their expiration date';